...
1) caricare i dati del file in una lista verificando la coerenza dei dati (numero di esami superati e 
numero di esami sostenuti devono essere maggior o uguali a 0 e il numero di esami superati 
deve essere minore o uguale al numeri di esami sostenuti)
e stampare la lista
...
int inserisciTesta(TLista* Pl, Tstudente Studente) {
    ...
    while (Corrente != NULL) 
    {
        if (strcmp(Corrente->info.matricola, Studente.matricola) == 0) 
        {
            trovato = 1;
            break;
        }
...
TLista cancellaStudenti(TLista L) {
    ...
    if (L == NULL)
        return L;
    TNodo* aux;
    if (L->info.Esamisostenuti == 0)
    {
        aux = L;
        L = L->next;
        free(aux);
    }
    L->next = cancellaStudenti(L->next);
    return L;
}
...
Tstudente* CreaElenco(TLista L, float P, int* dim) {
    ...
    d = conta(L, P);
    (*dim) = d;
    E = malloc(sizeof(Tstudente) * d);
    aux = L;
    int index = 0;
    while (aux != NULL)
    {
        if (((float)aux->info.Esamisuperati / aux->info.Esamisostenuti) > P)
            E[index++] = aux->info;
        aux = aux->next;
    }
    return E;
...
{
    //da implementare
    float SommaTotRate = 0;

    int NStudenti = 0;
    while (L != NULL) 
    {
        SommaTotRate += ((float)(L->info.Esamisuperati) / (float)(L->info.Esamisostenuti));
        NStudenti++;
        L = L->next;
    }
    float RateMedio;
    RateMedio = (float)(SommaTotRate / NStudenti);
    return RateMedio;
}
