...
float* elabora_vettore(int* v1, int* v2, int n, int* dim)
{
	...
	while (index < (*dim)) 
	{
		if (v2[j] != 0) 
		{
			vout[index++] = (float)((float)(2 * v1[j]) / (float)(v2[j]));
		}
		j++;
	}
	return vout;
...
int main(void) {
	...
	int dimv1;
	int dimv2;
	dimv1 = acquisisci_dati(v1, dim);
	dimv2 = acquisisci_dati(v2, dim);
	//4. stampare v1 e v2
	stampa_vettore(v1, dimv1);
	stampa_vettore(v2, dimv2);
