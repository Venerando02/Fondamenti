...
float* genera(float vettore[], int dim, int* nuova_dim)
{
    float* vettore_uscita;
    if (dim % 2 == 0) 
    {
        (*nuova_dim) = (dim / 2);
    }
    ...
    vettore_uscita = malloc(sizeof(float) * (*nuova_dim));
    for (int i = 0; i < *nuova_dim; i++) 
    {
        if (i == *nuova_dim - 1 && dim % 2 != 0) 
        {
            vettore_uscita[i] = vettore[i];
        }
        else {
            vettore_uscita[i] = vettore[i] + vettore[dim - 1 - i];
        }
    }
