...
// parametri:
//				c : coda da svuotare
void coda_svuota(coda_pazienti_t* pcoda)
{
	// FUNZIONE DA IMPLEMENTARE A CURA DELLO STUDENTE
	paziente_t* aux;
	while (!coda_vuota(*pcoda)) 
	{
		coda_cancella_primo(pcoda);
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////
...
int	carica_da_file(FILE * f, coda_pazienti_t* c) 
{
	paziente_t	p;
	int contatore = 0;
	if (f == NULL)
...
coda_pazienti_t* costruisci_coda_attesa(coda_pazienti_t* pcoda) 
{
	// FUNZIONE DA IMPLEMENTARE A CURA DELLO STUDENTE
	coda_pazienti_t* vettore_code = (coda_pazienti_t*)malloc(sizeof(coda_pazienti_t)*NUMERO_CODICI);
	for (int i = 0; i < NUMERO_CODICI; i++) 
	{
		vettore_code[i].primo = vettore_code[i].ultimo = NULL;
	}
	if (vettore_code == NULL)
		return NULL;
	while (!coda_vuota(*pcoda)) 
	{
		coda_inserisci(&vettore_code[pcoda->primo->codice], (*pcoda->primo));
		coda_cancella_primo(pcoda);
	}
	return vettore_code;
}
...
int calcolo_tempo_attesa(coda_pazienti_t* vettore, int dim, paziente_t p)
{
	// FUNZIONE DA IMPLEMENTARE A CURA DELLO STUDENTE
	int tempo = 0;
	coda_pazienti_t CodaAttesa;
	for (int i = 0; i < dim; i++)
	{
		CodaAttesa = vettore[i];
		while (CodaAttesa.primo != NULL)
		{
			if (confronta_pazienti(*CodaAttesa.primo, p) == 0)
			{
				tempo += (TEMPO_ATTESA_PER_CODICE[CodaAttesa.primo->codice] * 60);
			}
			CodaAttesa.primo = CodaAttesa.primo->successivo;
		}
	}
	return tempo;
}
...
void calcola_lunghezza_code(coda_pazienti_t vcode[], int dim, int* vettore) 
{
	// FUNZIONE DA IMPLEMENTARE A CURA DELLO STUDENTE
	int cont;
	coda_pazienti_t c;
...
int main(void) {
	...
	do {
		...
		case 1:
			...
			// Inserire il codice
			n = carica_da_file(fp, &coda_ingresso);
		...
		case 3:
			...
			int t_in_secondi = calcolo_tempo_attesa(lista_attesa, NUMERO_CODICI, paziente);
			// Attenzione il tempo deve essere stampato nel formato ORA:MINUTI
			printf("Il tempo in secondi e' %d.\n\n", t_in_secondi);
			// Inserire il codice
			break;
		case 4:
			...
			calcola_lunghezza_code(lista_attesa, NUMERO_CODICI, numero_pazienti_in_attesa);
			for (int codice = 0; codice < NUMERO_CODICI; ++codice)
