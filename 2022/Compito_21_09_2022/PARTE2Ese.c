...
lista_evento_t aggiungi_ordine(lista_evento_t l, evento_t ev)
{
    ...
    if (l == NULL || strcmp(ev.ora, l->ev.ora) < 0)
    {
        l = aggiungi(l, ev);
        return l;
    }
...
{
    // A CURA DELLO STUDENTE
    lista_evento_t l = NULL;
    evento_t aux;
    while (fscanf(f, "%d%s%d%s", &aux.id, aux.ora, &aux.potenza, aux.stato) == 4 && (aux.id <= NUMERO_MASSIMO_DISPOSITIVI) && (aux.id >= 0))
    {
        l = aggiungi(l, aux);
    }
    return l;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // distribuisci
  //    la funzione copia i valori presenti nella lista in ingresso nel vettore di liste ordinate in base al
...
float calcolo_consumo_dispositivo(vettore_eventi_t v, int id)
{
    ...
    while (aux != NULL)
    {
        if (aux != NULL && strcmp("ON", aux->ev.stato) == 0)
        {
            var = aux->ev;
            aux = aux->n;
            if (strcmp(aux->ev.stato, "OFF") == 0)
            {
                tempi = tempo(var, aux->ev);
                scalo = tempi / 60.0F;
                energia = energia + scalo * aux->ev.potenza;
            }
            else
            {
                break;
...
float* calcolo_consumi(vettore_eventi_t v)
{
    ...
    float ore;
    float energia = 0.0F;
    float* vettore_consumo = malloc(sizeof(float) * NUMERO_MASSIMO_DISPOSITIVI);
    ...
    for (int i = 0; i < NUMERO_MASSIMO_DISPOSITIVI; i++)
    {
        ...
        while (aux != NULL)
        {
            if (aux != NULL && strcmp("ON", aux->ev.stato) == 0)
            {
                evento = aux->ev;
                aux = aux->n;
                if (strcmp("OFF", aux->ev.stato) == 0)
                {
                    minuti = tempo(evento, aux->ev);
                    ore = minuti / 60.0F;
                    energia = energia + aux->ev.potenza * ore;
                }
                else
                {
                    break;
        ...
        vettore_consumo[i] = energia;
    }
    return vettore_consumo;
}
int main(void) {
    char nome_file[64];
    int id_dispositivo;
    ...
    do {
        ...
        case 1:
            ...
            {
                printf("Non hai inizializzato la lista.");
                return -2;
            }
            printf("---------LISTA RISULTANTE----------\n");
            stampa(valori_caricati);
            printf("\n\n");
            break;
        ...
        case 2:
            ...
            if (valori_caricati == NULL) 
            {
                printf("RITORNA ALL'OPZIONE 1\n");
                break;
            }
            for (int i = 0; i < NUMERO_MASSIMO_DISPOSITIVI; i++)
            {
                if (dispositivi[i] != NULL) 
                {
                    printf("NON HAI INIZIALIZZATO IL VETTORE\n");
                    break;
                }
            }
            distribuisci(valori_caricati, dispositivi);
            printf("-----------VETTORE DISPOSITIVI RISULTANTE--------\n");
            for (int i = 0; i < NUMERO_MASSIMO_DISPOSITIVI; i++) 
            {
                printf("DISPOSITIVO %d\n", i+1);
                stampa(dispositivi[i]);
                printf("\n");
            }
            printf("\n\n");
            break;
        case 3:
            ...
            vettore_consumo = calcolo_consumi(dispositivi);
            printf("----------VETTORE CONSUMO---------\n");
            for (int i = 0; i < NUMERO_MASSIMO_DISPOSITIVI; i++) 
            {
                printf("DISPOSITIVO %d : %f\n", i+1,vettore_consumo[i]);
            }
            break;
        case 4:
            // IMPLEMENTARE QUANTO NECESSARIO A CALCOLARE E STAMPARE IL CONSUMO DI UN QUALSIASI
            // DISPOSITIVO RICHIESTO DALL'UTENTE
            printf("Inserisci l'id di un dispositivo: ");
            scanf("%d", &id_dispositivo);
            consumo = calcolo_consumo_dispositivo(dispositivi, id_dispositivo);
            printf("\nIl consumo del dispositivo %d e' %f", id_dispositivo, consumo);
            printf("\n\n");
            break;
        }
    } while (scelta != 0);
