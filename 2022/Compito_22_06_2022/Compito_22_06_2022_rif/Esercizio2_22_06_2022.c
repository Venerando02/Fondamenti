...
int CaricaAuto(char* nome_file, ListaModelli* pLM) {
    ...
    TAuto automobile;
    char Modello[32];
    ListaModelli ListaOut = NULL;
    while (fscanf(fp, "%s%s%d", Modello, automobile.targa, &automobile.km_percorsi) == 3) 
    {
        ListaOut = AggiungiModello(pLM, Modello);
        if (!(AggiungiAuto(&(ListaOut->listaA), automobile))) 
        {
            return 0;
        }
    }
...
ListaAuto CreaListaVetture(ListaModelli LM, int soglia) {
    ...
    while (LM != NULL) 
    {
        ...
        while (aux != NULL) 
        {
            ...
            {
                AggiungiAuto(&ListaOut, aux->info);
            }
            aux = aux->succ;
        }
        LM = LM->succ;
...
void VisualizzaListaAuto(ListaAuto L) {
    while (L != NULL) {
        printf("targa = %s, km = %d\n", L->info.targa, L->info.km_percorsi);
        L = L->succ;
    }
}
/*
