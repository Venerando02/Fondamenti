...
Cassa* carica_casse(char* nome_file, int* numero_casse)
{
    ...
    if (fp == NULL)
        return NULL;
    if (fscanf(fp, "%d", numero_casse) != 1)
    {
        fclose(fp);
        return NULL;
    }
    Cassa* VettoreCasse = (Cassa*)malloc(sizeof(Cassa) * (*numero_casse));
    if (VettoreCasse == NULL)
        return NULL;
    for (int i = 0; i < (*numero_casse); i++)
    {
        inizializza_coda(&VettoreCasse[i].coda);
        if (fscanf(fp, "%d%s", &(VettoreCasse[i].id), VettoreCasse[i].impiegato) != 2)
        {
            fclose(fp);
            return NULL;
        }
    ...
    {
        if (fscanf(fp, "%d%s%f", &Cassa, c.nome, &c.tempo_stimato) != 3) 
        {
            fclose(fp);
            return NULL;
        }
        if (!aggiungi_cliente(&VettoreCasse[Cassa].coda, c))
            return NULL;
        else 
        {
            VettoreCasse[Cassa].coda.numero_clienti++;
        }
    }
    return VettoreCasse;
}
...
float* tempo_stimato(Cassa* casse, int dim) 
{
    ...
    if (v == NULL)
        return NULL;
    CodaClienti CodaCassa;
    float tcont;
    for (int i = 0; i < dim; i++) 
    {
        tcont = 0;
        CodaCassa = casse[i].coda;
        while (CodaCassa.primo != NULL) 
        {
            tcont += CodaCassa.primo->cliente.tempo_stimato;
            CodaCassa.primo = CodaCassa.primo->succ;
        }
        v[i] = tcont;
    }
    return v;
...
ListaClienti operazioni_lunghe(Cassa* casse, int dim, float tempo_minimo)
{
    ...
    for (int i = 0; i < dim; i++) 
    {
        ...
        while (C.primo != NULL) 
        {
            if (C.primo->cliente.tempo_stimato >= tempo_minimo) 
            {
                if (!aggiungi_cliente_a_lista(&ListaOut, C.primo->cliente))
                    return NULL;
            }
...
int main(void) {
    ...
    do {
        ...
        case 1:
            ...
            scanf("%s", NomeFile);
            casse = carica_casse(NomeFile, &ncasse);
            if (casse != NULL)
                visualizza_casse(casse, ncasse);
            break;
        ...
        case 2:
            if (casse == NULL) {
                ...
                clienti = operazioni_lunghe(casse, ncasse, tmin);
                visualizza_clienti(clienti);
                printf("\n");
                disalloca_clienti(&clienti);
            }
            break;
        ...
        case 3:
            if (casse == NULL) {
                ...
                tcasse = tempo_stimato(casse, ncasse);
                // TODO Implementazione a cura dello studente
                printf("--------\n");
