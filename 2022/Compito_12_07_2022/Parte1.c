...
float* crea_vettore(float* vettore_in, int dim, int* dim_out) 
{
    ...
    for (int i = 0; i < dim; i++) 
    {
        if (vettore_in[i] > 0) 
        {
            somma = somma + vettore_in[i];
            (*dim_out)++;
        }
    }
    float* v_out = malloc(sizeof(float) * (*dim_out));
    int index = 0;
    for (int i = 0; i < dim; i++) 
    {
        if (vettore_in[i] > 0) 
        {
            v_out[index++] = (vettore_in[i] / somma);
        }
    }
    return v_out;
}
...
int main(void) {
    ...
    // TODO STUDENTE
    vettore_positivi_scalati = crea_vettore(vettore, dimensione_array, &dim_array_positivi_scalati);

    // Se il vettore non e' stato allocato, esce
    // TODO STUDENTE
    if (vettore_positivi_scalati == NULL) 
    {
        return -1;
    }
    ...
    // TODO STUDENTE
    indice_max = cerca_massimo(vettore_positivi_scalati, dim_array_positivi_scalati);
