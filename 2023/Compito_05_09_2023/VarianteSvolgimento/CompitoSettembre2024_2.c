...
void lista_libera(anagrafe_lista * Plista) 
{
	// Da implementare
	anagrafe_t* aux;
	while (Plista->radice != NULL) 
	{
		aux = Plista->radice;
		Plista->radice = Plista->radice->p;
		free(aux);
	}
}
...
int lista_anagrafe_aggiungi_ordinato(anagrafe_lista* p_lista, anagrafe_t a) 
{
	...
	if ((p_lista->radice == NULL) || (strcmp(a.data, p_lista->radice->data) >= 0))
		return lista_anagrafe_aggiungi(p_lista, a);
	else
...
void crea_vettore(anagrafe_lista lista_ingresso, anagrafe_lista** vettore_uscita)
{
	...
	if ((*vettore_uscita) == NULL)
		return;
	for (int i = 0; i < 3; i++) 
	{
		(*vettore_uscita)[i].radice = NULL;
	}
	while (lista_ingresso.radice != NULL) 
	{
		if (lista_ingresso.radice->sesso == MASCHIO)
		{
			lista_anagrafe_aggiungi_ordinato(&(*vettore_uscita)[MASCHIO], *(lista_ingresso.radice));
		}
		else if (lista_ingresso.radice->sesso == FEMMINA)
		{
			lista_anagrafe_aggiungi_ordinato(&(*vettore_uscita)[FEMMINA], *(lista_ingresso.radice));
		}
		else 
		{
			lista_anagrafe_aggiungi_ordinato(&(*vettore_uscita)[ALTRO], *(lista_ingresso.radice));
		}
		lista_ingresso.radice = lista_ingresso.radice->p;
	}
}
...
//		
void estrai_giovane_vecchio(anagrafe_lista Lista, anagrafe_t* Giovane, anagrafe_t* Vecchio) 
{
	// Da implementare
	(*Giovane) = *(Lista.radice);
	(*Vecchio) = *(Lista.radice);
	while (Lista.radice != NULL) 
	{
		if (strcmp(Lista.radice->data, (*Giovane).data) > 0) 
		{
			(*Giovane) = *(Lista.radice);
		}
		if (strcmp(Lista.radice->data, (*Vecchio).data) < 0)
		{
			(*Vecchio) = *(Lista.radice);
		}
		Lista.radice = Lista.radice->p;
	}
}
...
int main(void) {
	...
	do {
		...
		case 1: {
			...
			scanf("%s", filename);
			lista_libera(&anagrafe);
			FILE* fp = fopen(filename, "r");
			if (fp == NULL)
				return -1;
			anagrafe = anagrafe_carica(fp);
			anagrafe_t* ListaAux = anagrafe.radice;
			while (ListaAux != NULL) 
			{
				anagrafe_stampa(ListaAux);
				ListaAux = ListaAux->p;
			}
		...
		case 2:
			...
			// Da completare in base alla descrizione e alle specifiche
			printf("Il piu' giovane e' %s con data %s, il piu' vecchio e' %s con data %s.\n", Giovane.cf, Giovane.data, Vecchio.cf, Vecchio.data);
			break;
		case 3:
			...
			{
				printf("SESSO : %s\n", CONVERSIONE_SESSO[i]);
				while (vettore[i].radice != NULL) 
				{
					anagrafe_stampa(vettore[i].radice);
					vettore[i].radice = vettore[i].radice->p;
					printf("\n");
				}
			}
