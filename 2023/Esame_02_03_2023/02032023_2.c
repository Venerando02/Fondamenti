...
/*
1) CaricaEventi:
	Carica i dati dal file in un vettore di N liste, in cui la lista i-esima contiene solo gli eventi dell'ambiente 
	i-esimo. La funzione restituisce il numero totale di eventi caricati o -1 in caso di errore durante il caricamento.
...
/*
2) EstraiEvento:
 Dato il vettore di liste di eventi ed un identificativo di ambiente, estrae e restituisce il 
 primo evento presente nella corrispondente lista.
...
/*
3) CercaEvento:
dato il nome di un evento, ricercare tutti gli eventi con quel nome nel vettore di liste e ricopiarli in un
opportuno vettore fornito in uscita.
...
TEvento* CercaEvento(char* nome_evento, PNodoListaEvento veventi[], int* n_eventi) 
{
	...
	TEvento* vettore = (TEvento*)malloc(sizeof(TEvento) * dim);
	if (vettore == NULL)
		return NULL;
	for (int i = 0; i < N; i++)
	{
		...
		while (ListaAux != NULL)
		{
			if (strcmp(ListaAux->info.nome, nome_evento) == 0)
			{
				vettore[(*n_eventi)++] = ListaAux->info;
			}
			ListaAux = ListaAux->next;
		}
	}
...
int main(void) {
	...
	int n, k;
	int ambiente;
	TNodoListaEvento* eventoestratto = NULL;
	char nome_evento[32];
	char NomeFile[64];
	TEvento* vett_eventi = NULL;
	...
	do {
		...
		case 1:
			...
			if (n == -2)
				...
				printf("Errore durante il caricamento dei dati sulla struttura.\n");
			else
				printf("%d caricati con successo.\n", n);
		...
		case 2:
			...
			printf("L'evento estratto e' %s %s %d.\n", eventoestratto->info.data, eventoestratto->info.nome, eventoestratto->info.ambiente);
			break;
