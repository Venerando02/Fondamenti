...
int ParcheggioAuto(TListaParcheggio v[], int dim, char* targa, char* orario, int piano) 
{
	// DA IMPLEMENTARE A CURA DELLO STUDENTE
	TPostoAuto aux;
	aux.idposto = TrovaPostoDisponibile(v[piano]);
	strcpy(aux.oraingresso, orario);
	strcpy(aux.orauscita, "");
	strcpy(aux.targa, targa);
	if (!AggiungiNodoLista(&v[piano], aux))
		return -1;
	else
		return (aux.idposto);
}
...
/*
2) ContaAuto:
funzione che, dato in ingresso il vettore dei parcheggi, 
restituisca in un opportuno vettore di interi il numero di auto attualmente presenti per ciascun piano
...
int* ContaAuto(TListaParcheggio v[], int dim) 
{
	...
	TListaParcheggio ListaParcheggio;
	for (int i = 0; i < dim; i++) 
	{
		cont = 0;
		ListaParcheggio = v[i];
		while (ListaParcheggio != NULL) 
		{
			cont++;
			ListaParcheggio = ListaParcheggio->next;
		}
		vout[i] = cont;
	}
	return vout;
...
int UscitaAuto(TListaParcheggio v[], int dim, char* targa, char* orario) 
{
	...
	for (int i = 0; i < dim; i++) 
	{
		...
		while (ListaParcheggio != NULL) 
		{
			if (strcmp(ListaParcheggio->info.targa, targa) == 0) 
			{
				strcpy(ListaParcheggio->info.orauscita, orario);
				ListaParcheggio->info.idposto = -1;
				return 1;
			}
			ListaParcheggio = ListaParcheggio->next;
		}
...
int main(void) {
	...
	do {
		...
		case 1:
			...
			printf("Inserisci il piano su cui inserire l'auto: ");
			scanf("%d", &piano);
			printf("Inserisci la targa dell'auto: ");
			scanf("%s", targa);
			printf("Inserisci l'orario di ingresso dell'auto: ");
			scanf("%s", orario);
			n = ParcheggioAuto(vettore, N, targa, orario, piano);
			printf("L'auto è parcheggiata nel posto %d.\n", n);
			break;
		...
		case 3:
			...
			printf("Inserisci la targa dell'auto: ");
			scanf("%s", targa);
			printf("Inserisci l'orario di uscita dell'auto: ");
			scanf("%s", orario);
			esito = UscitaAuto(vettore, N, targa, orario);
			if (esito == 1)
				printf("Dati correttamente modificati.\n");
			else
