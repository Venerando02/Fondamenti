...
* Per ogni idposto considerato, bisogna però controllare che il posto sia libero, ossia non occupato da un'auto parcheggiata in quel medesimo posto.
...
{
	// DA IMPLEMENTARE A CURA DELLO STUDENTE
	if ((piano >= 0) && (piano < dim))
	{
		TListaParcheggio parcheggio = v[piano];
		TPostoAuto veicolo;
		strcpy(veicolo.oraingresso, orario);
		strcpy(veicolo.targa, targa);
		veicolo.idposto = TrovaPostoDisponibile(v[piano]);
		if (!AggiungiNodoLista(&v[piano], veicolo))
			return -1;
		else return veicolo.idposto;
	}
	return -1;
}
...
{
	// DA IMPLEMENTARE A CURA DELLO STUDENTE
	int* vettore_parcheggi = malloc(sizeof(int) * dim);
	int cont;
	for (int i = 0; i < dim; i++)
	{
		cont = 0;
		...
		vettore_parcheggi[i] = cont;
	}
	return vettore_parcheggi;
}
...
{
	// DA IMPLEMENTARE A CURA DELLO STUDENTE
	TListaParcheggio aux;
	for (int i = 0; i < dim; i++) 
	{
		aux = v[i];
		while (aux != NULL) 
		{
			if ((strcmp(aux->info.targa, targa) == 0) && (aux->info.idposto != -1)) 
			{
				strcpy(aux->info.orauscita, orario);
				aux->info.idposto = -1;
				return 1;
			}
			aux = aux->next;
...
int main(void) {
	...
	do {
		...
		case 1:
			...
			printf("\nInserisci il piano: ");
			scanf("%d", &piano);
			esito = ParcheggioAuto(vettore, N, targa, orario, piano);
			printf("\nL'auto e' stata parcheggiata nel posto %d", esito);
			printf("\n");
			break;
