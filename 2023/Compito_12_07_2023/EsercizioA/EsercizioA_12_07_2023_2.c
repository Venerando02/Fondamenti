...
void lista_segmenti_aggiungi(lista_segmenti* pl, segmento_t segmento)
{
	...
	segmento_t* aux = (segmento_t*)malloc(sizeof(segmento_t));
	if (aux == NULL)
		return;
	aux->estremo_a = segmento.estremo_a;
	aux->estremo_b = segmento.estremo_b;
	aux->successivo = (*pl).radice;
	(*pl).radice = aux;
	pl->numero_elementi++;
	return;
}
...
//		void
#include <math.h>
void segmenti_lunghezza(punto_t A, punto_t B , float * Distanza) {
	// Funzione da implementare a cura dello studente
	float distanza = 0.0;
	int dCoordinataX;
	int dCoordinataY;
	dCoordinataX = A.coordinata_x - B.coordinata_x;
	dCoordinataY = A.coordinata_y - B.coordinata_y;
	distanza = sqrt((dCoordinataX * dCoordinataX) + (dCoordinataY * dCoordinataY));
	(*Distanza) = distanza;
}
...
{
	// Funzione da implementare a cura dello studente
	(*p_lista_uno).radice = NULL;
	(*p_lista_due).radice = NULL;
	float distanza = 0.0;
	segmento_t* aux;
	while (lista_in.radice != NULL) 
	{
		segmenti_lunghezza(lista_in.radice->estremo_a, lista_in.radice->estremo_b, &distanza);
		aux = lista_in.radice;
		if (distanza < soglia) 
		{
			lista_segmenti_aggiungi(p_lista_uno, (*aux));
		}
		else
		{
			lista_segmenti_aggiungi(p_lista_due, (*aux));
		}
		lista_in.radice = lista_in.radice->successivo;
	}
}
...
int main(void) {
	...
	lista_segmenti Lista1 = { NULL, 0 };
	lista_segmenti Lista2 = { NULL, 0 };
	float soglia;
	float d;
	int selezione;
	do {
		...
		case 1: {
			...
			scanf("%s", filename);
			segmenti = segmenti_carica(filename);
			segmenti_stampa(segmenti);
		...
		case 2:
			...
			printf("Sono presenti all'interno della lista %d elementi.\n", segmenti.numero_elementi);
			// Completamento a cura dello studente
		...
		case 3: {
			...
			// Completamento a cura dello studente
			segmenti_dividi(segmenti, &Lista1, &Lista2, soglia);
			printf("LISTA 1:\n");
			segmenti_stampa(Lista1);
			printf("-----------------\nLISTA 2:\n");
			segmenti_stampa(Lista2);
			break;
		}
		case 4: {
			// stampa le lunghezze di tutti i segmenti contenuti nella lista
			int index = 0;
			float* VSegmenti = (float*)malloc(sizeof(float)*segmenti.numero_elementi);
			if (VSegmenti == NULL)
				return 0;
			while (segmenti.radice != NULL) 
			{
				segmenti_lunghezza(segmenti.radice->estremo_a, segmenti.radice->estremo_b, &d);
				VSegmenti[index++] = d;
				segmenti.radice = segmenti.radice->successivo;
			}
			for (int i = 0; i < segmenti.numero_elementi; i++) 
			{
				printf("v[%d] = %.3f\n", i, VSegmenti[i]);
			}
			// Completamento a cura dello studente
