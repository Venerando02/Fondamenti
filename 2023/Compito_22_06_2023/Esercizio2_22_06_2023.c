...
int carica_file(const char* nomefile, coda_t* pc) {
	...
	if (fp == NULL)
		return 0;
	evento_t ev;
	while (fscanf(fp, "%s%d%d", ev.data, &ev.tipo, &ev.rilevanza) == 3) 
	{
		if (!coda_aggiungi(pc, ev))
			return 0;
	}
	fclose(fp);
	return 1;
...
lista_t crealista_maggiorisoglia(lista_t *v, int dim, int soglia)
{
	// DA IMPLEMENTARE A CURA DELLO STUDENTE
	lista_t ListaOut = NULL;
	lista_t aux;
	for (int i = 0; i < dim; i++) 
	{
		aux = v[i];
		while (aux != NULL) 
		{
			if (aux->evento.rilevanza > soglia) 
			{
				if (!lista_aggiungi_ordine(&ListaOut, aux->evento))
					return NULL;
			}
			aux = aux->successivo;
		}
...
void VisualizzaVettore(lista_t* Vettore, int dim) 
{
	lista_t aux;
	for (int i = 0; i < dim; i++) 
	{
		printf("TIPOLOGIA EVENTO: %d\nELENCO:\n", i);
		aux = Vettore[i];
		while (aux != NULL) 
		{
			printf("%d %s %d\n", aux->evento.tipo, aux->evento.data, aux->evento.rilevanza);
			aux = aux->successivo;
		}
		printf("\n");
	}
...
int main(void) {
	...
	do {
		...
		case 1: {
			...
			scanf("%s", NomeFile);
			e = carica_file(NomeFile, &c);
			if (e == 1)
				printf("Caricamento dati da file effettuato con successo.\n");
			break;
		}
		case 2: {
			...
			while (c.primo != NULL) 
			{
				e = coda_primo(c, &EventoDaCancellare);
				if (e != 0)
				{
					if (lista_aggiungi_ordine(&v[EventoDaCancellare.tipo], EventoDaCancellare) == 1)
						coda_rimuovi(&c);
				}
			}
			printf("\n");
			VisualizzaVettore(v, 10);
			break;
		}
		case 3: {
		...
		case 4: {
			...
			scanf("%d", &soglia);
			ListaOut = crealista_maggiorisoglia(v, 10, soglia);
			while (ListaOut != NULL) 
			{
				if (ListaOut->evento.tipo == tipo) 
				{
					printf("%s %d %d\n", ListaOut->evento.data, ListaOut->evento.tipo, ListaOut->evento.rilevanza);
				}
				ListaOut = ListaOut->successivo;
			}
			break;
		}
		default: {
