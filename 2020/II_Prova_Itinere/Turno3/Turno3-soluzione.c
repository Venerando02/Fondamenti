...
I dati delle partite sono memorizzati in modo persistente in un file di testo e gestiti
in memoria centrale.
...
1.	carica_dati: caricamento dei dati dal file in due liste, la prima contiene le partite, la seconda contiene
	l'elenco dei giocatori con i relativi punteggi  ordinata dal
	punteggio migliore a quello peggiore (punteggio medio per partita).
	Nota che in fase di caricamento, qualora il giocatore
	sia già presente (nella lista dei giocatori), il punteggio deve essere aggiornato.
2.	carica_partita: Caricamento di una nuova partita da console (e aggiornamnto del punteggio giocatore)
...
// La funzione calcola il punteggio medi del giocatore 
//	parametri in ingresso:
...
t_giocatore* giocatore_cerca(t_lista_giocatori lista, char nickname[]) {
	while (lista != NULL) {
		if (strcmp(nickname, lista->giocatore.giocatore) == 0)
...
void carica_partita(t_lista_partite* pt_partite, t_lista_giocatori* pt_giocatori, t_partita p) {
	t_giocatore g;
	t_giocatore* pt_g;
	*pt_partite = partita_aggiungi(*pt_partite, p);
	...
	if (pt_g == NULL) {
		...
		*pt_giocatori = giocatore_aggiungi_ordinato(*pt_giocatori, g);
		...
		++(pt_g->numero_partite_giocate);
		pt_g->punteggio_totale += p.punteggio_a;
	}
	pt_g = giocatore_cerca(*pt_giocatori, p.giocatore_b);
	if (pt_g == NULL) {
		...
		*pt_giocatori = giocatore_aggiungi_ordinato(*pt_giocatori, g);
		...
		++(pt_g->numero_partite_giocate);
		pt_g->punteggio_totale += 120 - p.punteggio_a;
	}
...
void carica_dati(const char* nome_file, t_lista_partite* pt_partite, t_lista_giocatori* pt_giocatori) {
	...
	if (f == NULL) {
		*pt_partite = *pt_giocatori = NULL;
		return; // Segnalazione di errore
	}
...
int main(void) {
	...
	do {
		...
		case 1:
			...
			// 3. visualizza le partite presenti in lista
