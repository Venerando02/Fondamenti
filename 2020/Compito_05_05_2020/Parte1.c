...
// Musumeci Venerando Pio M-Z 1000015141
...
void Carica(int* v, int n) {
	...
	while (i < n) 
	{
		printf("Inserisci il valore %d (positivo e pari): ", i);
		scanf("%d", &v[i]);
		if ((v[i] % 2 == 0) && (v[i] >= 0))
		{
			i++;
...
int* Select(int* v, int dim1, int* dim2) {
	/* A partire dal vettore v di dimensione dim1, restituisce un nuovo vettore contenente 
	tutti gli elementi del vettore v che sono multipli di 5.
	...
	int* v_out = malloc(sizeof(int) * (*dim2));
	if ((*dim2) == 0)
	{
		printf("Non ci sono multipli di 5");
		return NULL;
	}
	int index = 0;
	for (int i = 0; i < dim1; i++) 
...
int main(void) {
	...
	v1 = malloc(sizeof(int)*dim1);             // Alloca un array di dim1 elementi int
