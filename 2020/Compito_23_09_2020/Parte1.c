...
void Bin2Hex(char* bin_str, char* hex_str)
{
    ...
    char cifra[5];
    int dim = strlen(bin_str);
    int index = 0;
    // devo fare avanzare il ciclo di 4
    // metto la terminazione poi utilizzo la f BITS4HEX per trovare il corrispondente esadecimale
    for (int i = 0; i < dim; i = i+4) 
    {
        strncpy(cifra, bin_str+i, 4);
        cifra[4] = '\0';
        hex_str[index++] = Bits4Hex(cifra);
    }
    hex_str[index] = '\0';
}
 ...
 */
void VisualizzaHex(char* stringa, int dim, int n)
{
    // ...implementare a cura dello studente...
    int count = 0;
...
int main(void)
{
    ...
    */
    // ...istruzioni...
    n = AcquisisciBinario(bin_str);
    if (n != 1) 
    {
        printf("La stringa non rispetta il requisito di avere solo 0 e 1.");
    }
    ...
    */
    //...istruzioni...
    int dim = (strlen(bin_str)/4)+1;
    hex_str = malloc(sizeof(char) * (dim));
