...
/*
  ...
  per npunti volte.
 */
float* ValutaPolinomio(float min, float max, int npunti, int grado, float* coeff)
{
    /* Implementare la funzione */
    float* valori = (float*)malloc(sizeof(float) * npunti);
    if (valori == NULL)
        return NULL;
    float passo = (float)((max - min) / npunti);
    for (int i = 0; i < npunti; i++) 
    {
        float x = min + i * passo;
        valori[i] = ValutaPolinomioX(x, grado, coeff);
    }
    return valori;
}
...
int main(void)
{
    ...
    coefficienti = AcquisisciCoefficienti(grado);
    ...
    polinomio = ValutaPolinomio(min, max, npunti, grado, coefficienti);
    if (polinomio == NULL)
        return 1; // esce se non riesce ad allocare l'array dei punti del polinomio

    VisualizzaValori(polinomio, npunti);
